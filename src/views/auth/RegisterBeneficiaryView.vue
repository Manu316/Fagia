  ...
  <div class="container">
    ...
    <form @submit.prevent="submitBeneficiaryForm">
      ...
      <div class="form-group">
        ...
        <input type="url" v-model="beneficiary.website" />
      </div>
      <div class="form-group">
  ...
  methods: {
    async submitBeneficiaryForm() {
      ...
      try {
        ...
        this.errorMessage = error.response?.data?.error || error.response?.data?.message || 'Error al registrar beneficiario.';
        console.error(error);
      }
    }
  }
...
button[type="submit"] {
  background-color: #007bff; /* Diferente color para el botón de beneficiario */
  color: white;
  padding: 10px 15px;
  border: none;
  border-radius: 4px;
