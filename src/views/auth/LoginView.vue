<template>
  <div class="container"> <h2>Iniciar Sesión</h2>
...
import { useRouter } from 'vue-router'; // useRoute no se usa, así que lo quité de la importación
...
const handleLogin = async () => {
  ...
  try {
    ...
    // o si tu authStore.login devuelve algo para indicar éxito, puedes redirigir aquí.
    // Ejemplo: const success = await authStore.login(credentials.value);
    // if (success) {
    //   const redirectPath = route.query.redirect || (authStore.userRole === 'donator' ? '/donator/dashboard' : (authStore.userRole === 'beneficiary' ? '/beneficiary/dashboard' : '/'));
    ...
    if (err.response && err.response.data) {
        ...
        error.value = err.response.data.error || err.response.data.message || 'Credenciales incorrectas o error en el servidor.';
    } else {
...
onMounted(() => {
  if (authStore.isAuthenticated) {
    ...
    if (authStore.userRole === 'donator') {
      ...
      // Podría ser una página principal para usuarios logueados o UserDonations
      router.push('/donations'); // Ajusta esta ruta de fallback según sea necesario
    }
  }
});
</script>
...
/* .mx-2 { margin-left: 0.5rem; margin-right: 0.5rem; } */ /* Si usas el separador | */
</style>